...
function SurveyDateTime({id, setSaveNeeded, onChange, config, setInitialSetting}: CustomSettingChildComponentProp) {
    ...
    // Effect to load values from saved config
    useEffect(() => {
        const savedTimestamp = config.PluginSettings.Plugins['com.mattermost.user-survey']?.systemconsolesetting?.SurveyDateTime?.timestamp;
        if (savedTimestamp) {
